...
class MOSS_SAF_WP_Settings {
	...
	*/
	...
	function license_callback( $args ) {
		...
		$size = ( isset( $args['size'] ) && ! is_null( $args['size'] ) ) ? $args['size'] : 'regular';
		$html = '<input type="text" class="' . $size . '-text" id="moss_saf_settings[' . $args['id'] . ']" name="moss_saf_settings[' . $args['id'] . ']" value="' . esc_attr( stripslashes( $value ) ) . '"/>';
		$license_status = get_option('vat_moss_saf_license_active');

		$license_key = ! empty( $value ) ? $value : false;

		if( 'valid' === $license_status && ! empty( $license_key ) ) {
			$html .= '<input type="submit" class="button" name="moss_saf_deactivate_license" value="' . esc_attr__( 'Deactivate License', 'vat_moss_saf' ) . '"/>';
			$html .= '<span style="color:green;">&nbsp;' . __( 'Your license is valid!', 'vat_moss_saf' ) . '</span>';
			...
			$html .= '<a href="' . esc_url( $renewal_url ) . '" class="button-primary">' . __( 'Renew Your License', 'vat_moss_saf' ) . '</a>';
			$html .= '<br/><span style="color:red;">&nbsp;' . __( 'Your license has expired, renew today to continue getting updates and support!', 'vat_moss_saf' ) . '</span>';
		} else {
			$html .= '<input type="submit" class="button" name="moss_saf_activate_license" value="' . esc_attr__( 'Activate License', 'vat_moss_saf' ) . '"/>';
		}
